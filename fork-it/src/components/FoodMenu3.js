...
export default function FoodMenu3() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:3001/api/getFood3").then((response) => {
  ...
  const submitFoodItem = () => {
    Axios.post("http://localhost:3001/api/insertFood3", {
      Food: Food,
      Price: Price,
    });
    alert("Added to Menu!!!");
  ...
  const deleteFoodItem = (FoodID) => {
    Axios.delete(`http://localhost:3001/api/deleteFood3/${FoodID}`);
  };
