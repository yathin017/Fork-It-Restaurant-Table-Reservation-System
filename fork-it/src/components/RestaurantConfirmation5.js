...
export default function RestaurantConfirmation5() {
  const [CustomerList, setCustomerList] = useState([]);

  useEffect(() => {
    Axios.get("http://localhost:3001/api/get5").then((response) => {
  ...
  const deleteRow = (EmailId) =>{
    Axios.delete(`http://localhost:3001/api/delete5/${EmailId}`)
  }
