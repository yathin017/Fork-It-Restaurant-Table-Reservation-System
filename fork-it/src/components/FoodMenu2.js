...
export default function FoodMenu2() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:3001/api/getFood2").then((response) => {
  ...
  const submitFoodItem = () => {
    Axios.post("http://localhost:3001/api/insertFood2", {
      Food: Food,
      Price: Price,
    });
    alert("Added to Menu!!!");
  ...
  const deleteFoodItem = (FoodID) => {
    Axios.delete(`http://localhost:3001/api/deleteFood2/${FoodID}`);
  };
