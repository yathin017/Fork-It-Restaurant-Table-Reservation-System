...
export default function RestaurantConfirmation6() {
  const [CustomerList, setCustomerList] = useState([]);

  useEffect(() => {
    Axios.get("http://localhost:3001/api/get6").then((response) => {
  ...
  const deleteRow = (EmailId) =>{
    Axios.delete(`http://localhost:3001/api/delete6/${EmailId}`)
  }
